    ...
    return (
        ...
        <div className='flex-1 h-full flex justify-center items-center'>
            ...
            <div className='h-3/4 w-3/4 '>
                <div  className='h-full w-full bg-gray-500 rounded-lg shadow-lg flex flex-col gap-8 p-10'>
                    {data.map((e)=>{ return(
                    <div key={e.idSalon} className='flex flex-row gap-10'>
                        <div key={e.idSalon} className='bg-white w-[250px] rounded-lg text-center shadow-lg flex flex-row gap-5'>
                            <p key={e.idSalon} className='m-auto'>{e.name_salon}</p> 
                            <p key={e.idSalon} className='m-auto'>x messages</p>
                        </div>
                        <button key={e.idSalon} type='button' onClick={()=>handleChange(e)}  className='bg-blue-800 rounded-lg w-[150px] shadow-lg text-white'>Rejoindre</button>
                    </div>)
                    })}
                </div>
            </div>
